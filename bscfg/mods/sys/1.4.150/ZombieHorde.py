...
class ZombieHorde(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        settings = [("Lives Per Player",{'default':1,'minValue':1,'maxValue':10,'increment':1}),
                    ("最大僵尸数", {'default':10,'minValue':5, 'maxValue':50,'increment':5}),
    ...
    def getInstanceDescription(self):
        return '杀死僵尸得分，僵尸玩家为2分!' if isinstance(self.getSession(),bs.TeamsSession) else '杀死僵尸得分，僵尸玩家为2分!'
    ...
    def _onSpazBotDied(self,DeathMsg):
        ...
        if len(self._bots.getLivingBots()) < self.settings['最大僵尸数量']:
