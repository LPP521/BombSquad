...
def bsGetGames():
	return [RunaroundTeam]

class RunaroundTeam(bs.TeamGameActivity):
    ...
    @classmethod
    def getName(cls):
        return 'Runaround(Team)' if bs.getLanguage() != "Chinese" else u"团队塔防"

    @classmethod
    def getDescription(cls,sessionType):
        return "Prevent enemies from reaching the exit.\nMade by Plasma Boson."  if bs.getLanguage() != "Chinese" else u"阻止敌人到达终点.\nPlasma Boson制作."

    def getInstanceScoreBoardDescription(self):
        return (('Prevent enemies from reaching the exit\nwith your teammates.Infinity Mode.' if self.settings['Mode'] == 0 else 'Prevent enemies from reaching the exit\nwith your teammates.')
                if bs.getLanguage() != "Chinese" else (u'和你的队友阻止敌人到达终点.无限模式.' if self.settings['Mode'] == 0 else u'和你的队友阻止敌人到达终点.'))
    ...
    def getSettings(cls, sessionType):
        return [("Time Limit (Minutes)", {'minValue': 1, 'default': 15, 'increment': 1}),
                ("Lives", {'minValue': 1, 'default': 10, 'increment': 1}),
                ('Mode', {'choices':[('Pro', 1), ('Uber', 2),('Endless',0)],'default': 2}),
    ...
    def __init__(self, settings):
        ...
        self.settings['Time Limit (Minutes)'] = max(60, self.settings['Time Limit (Minutes)']*60)
    ...
    def onTransitionIn(self):
        ...
        self.setupStandardTimeLimit(self.settings['Time Limit (Minutes)'])
    ...
    def onBegin(self):
        ...
        self._startLives = max(1,self.settings['Lives'])
    ...
    def spawnPlayer(self,player):
        ...
        self.spawnPlayerSpaz(player,pos)
    ...
    def _updateWaves(self):
        ...
        if self._canEndWave and not self._bots.haveLivingBots() and not self._gameOver and self._lives > 0:
            ...
            if won:
                ...
                '''
                if self.s_preset in ['pro','proEasy']:
                ...
                self._gameOver = True
                bs.gameTimer(baseDelay,self.endGame)
                return
    ...
    def handleMessage(self,m):
        ...
        if isinstance(m,bs.PlayerScoredMessage):
            #print m.score
            #m.spaz.getPlayer().getTeam().gameData['score'] += int(m.score)
            pass#self._updateScores()
            ...
            if m.killerPlayer is not None:
                ...
                try:
                    if m.killerPlayer is not None and m.killerPlayer.exists():
                        ...
                        m.killerPlayer.getTeam().gameData['score'] += int(pts)
                        #self._updateScores()
    ...
    def __superHandleMessage(self,m):
        super(RunaroundTeam,self).handleMessage(m)
